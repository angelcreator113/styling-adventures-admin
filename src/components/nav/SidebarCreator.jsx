...
import {
  Home, CalendarDays, Pin, Instagram, Youtube,
  MessageCircle, Timer, Layers3, ChartNoAxesCombined,
} from "lucide-react";
...
export default function SidebarCreator({ collapsed = false }) {
  const groups = [
    {
      ...
      items: [
        { to: "/creator/home",      label: "Homepage",         icon: Home, end: true },
  ...
  return (
    <nav className={`sidebar-nav ${collapsed ? "is-collapsed" : ""}`} aria-label="Creator navigation">
      {groups.map((g) => (
        <div key={g.title}>
