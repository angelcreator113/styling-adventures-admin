  ...
  const groups = [
    {
      ...
      items: [
        ...
        { to: "/admin/site-messages", label: "Site Messages", icon: Megaphone },
    ...
    <nav className={"sidebar-nav " + (collapsed ? "is-collapsed" : "")} aria-label="Admin navigation">
      {groups.map((g) => (
        <div key={g.title} className="sidebar-section">
          {!collapsed && (<div className="sidebar-section-title" aria-hidden>{g.title}</div>)}
            ...
            {g.items.map(function (item) {
              ...
              return (
                <li key={item.to}>
                  <NavLink
                    to={item.to}
                    className={function ({ isActive }) { return "sidebar-link " + (isActive ? "is-active" : ""); }}
                  >
