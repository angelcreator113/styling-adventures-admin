...
// Best-effort: pull panel def so we honor accept/defaultVisibility without hardcoding
async function getPanelDef(panelId = 'episodes') {
  try {
...
export async function setupEpisodeUploadUI() {
  // Pull config; fall back to safe defaults (from seed-panel-registry)
  const panel = (await getPanelDef('episodes')) || {
    accept: 'video/*',
  ...
  const prefix = panel.uiPrefix || 'episode-';
  const acceptAttr = panel.accept || 'video/*';
  const defaultVisibility = panel.defaultVisibility === 'public' ? 'public' : 'private';

  // 1) Load category dropdowns with the correct tree
  loadCategoryData(panel.categoryPanelKey || 'episodes', prefix);
  ...
  if (!form || !fileInput || !dropArea) {
    console.error('[Episode Upload] Missing required DOM nodes.');
    return;
  }

  // Ensure accept attribute matches panel config (won’t break if already set in HTML)
  ...
  form.addEventListener('submit', async (e) => {
    ...
    // Parse accept list for MIME validation
    ...
    if (!mimeAllowed(file, acceptList)) {
      alert(`File type "${file.type || 'unknown'}" is not allowed. Allowed: ${acceptList.join(', ') || 'video/*'}`);
      return;
    }

    // Categories are optional; normalize empty strings
    const category = catEl?.value?.trim() || '';
    const subcategory = subcatEl?.value?.trim() || '';
    const subsubcategory = subsubcatEl?.value?.trim() || '';

    // Visibility: UI checkbox overrides; default to panel default if absent
    ...
    const path = `episodes/${userId}/${Date.now()}_${safeName}`;
    ...
    try {
      ...
      // Firestore mirror
      const col = collection(db, 'episodes');
      const episodeDocRef = doc(col); // auto-id
      const id = episodeDocRef.id;
      ...
      document.dispatchEvent(new CustomEvent('episodes:uploaded', { detail: { id, path } }));

      alert(`✅ Episode uploaded as ${visibility}.`);
    } catch (err) {
