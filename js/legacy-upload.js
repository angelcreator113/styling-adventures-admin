// script.js - Legacy standalone uploader
...
const firebaseConfig = {
  ...
  storageBucket: "styling-admin.appspot.com", // ✅ fixed
...
async function displayUploads(collectionName, listId, type) {
  ...
  snap.forEach(docSnap => {
    ...
    div.innerHTML = `
      ...
      ${type === 'link' ? `<a href="${data.url}" target="_blank">View</a>` : ''}
...
document.getElementById('episode-upload-btn')?.addEventListener('click', async () => {
  ...
  await addDoc(collection(db, "episodes"), { filename: file.name, url, type: "video" });
  alert("Episode uploaded!"); displayUploads('episodes', 'episode-list', 'link');
});
...
document.getElementById('voice-upload-btn')?.addEventListener('click', async () => {
  ...
  await addDoc(collection(db, "voice"), { filename: file.name, url, type: "audio" });
  alert("Voice file uploaded!"); displayUploads('voice', 'voice-list', 'link');
});
...
document.getElementById('closet-upload-btn')?.addEventListener('click', async () => {
  ...
  await addDoc(collection(db, "closet"), { filename: file.name, url, type: "image" });
  alert("Closet item uploaded!"); displayUploads('closet', 'closet-list', 'link');
});
...
document.getElementById('meta-publish-btn')?.addEventListener('click', async () => {
  ...
  alert("Metadata saved!"); displayUploads('metadata', 'meta-list', 'text');
});
